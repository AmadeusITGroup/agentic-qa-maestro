    ...
    try:
        with _get_jira_client() as client:
            ...
            if response.status_code == 200:
                ...
                return json.dumps(
                    {"total": len(comments), "comments": comments}, indent=2
                )
            else:
                return (
    ...
    try:
        with _get_jira_client() as client:
            ...
            if response.status_code == 200:
                ...
                return json.dumps(
                    {"total": data.get("total", 0), "issues": issues}, indent=2
                )
            else:
                return (
                    f"Error searching issues: "
                    f"HTTP {response.status_code} - {response.text}"
                )
    except Exception as e:
...
def jira_create_bug(
    ...
    description: Annotated[str, Field(description="Detailed bug description with steps to reproduce")],
...
def jira_transition_issue(
    ...
    transition_name: Annotated[str, Field(description="Target transition name, e.g. 'In Progress', 'Done'")],
) -> str:
...
JIRA_TOOLS = [
    jira_get_issue, jira_add_comment, jira_get_comments, jira_search_issues,
    jira_create_issue, jira_create_bug, jira_transition_issue,
]
