...
def _do_open_url(url: str):
    ...
    _page.wait_for_timeout(5000)  # SPA settle time
...
def _do_fill(selector: str, value: str):
    ...
    if value == "CREDENTIAL_USERNAME":
        resolved_value = os.environ.get(
            "CREDENTIAL_USERNAME", os.environ.get("APP_USERNAME", value)
        )
    elif value == "CREDENTIAL_PASSWORD":
        resolved_value = os.environ.get(
            "CREDENTIAL_PASSWORD", os.environ.get("APP_PASSWORD", value)
        )
...
def fill(
    ...
    value: Annotated[
        str,
        Field(
            description="Value to fill. Use CREDENTIAL_USERNAME or CREDENTIAL_PASSWORD for credential resolution"
        ),
    ],
) -> str:
...
def authenticate_aad(
    ...
    method: Annotated[
        str, Field(description="Auth injection method: header, token_url, easyauth, msal_cache")
    ] = "header",
    token_login_path: Annotated[
        str, Field(description="Path for token_url method")
    ] = "/auth/token",
    token_param_name: Annotated[
        str, Field(description="Query parameter name for token_url method")
    ] = "token",
) -> str:
